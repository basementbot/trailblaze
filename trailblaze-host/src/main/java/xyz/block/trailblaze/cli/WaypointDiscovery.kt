  ...
  private fun tryLoadWorkspaceWaypoints(
    ...
    return try {
      ...
      val resolved = TrailblazeProjectConfigLoader.loadResolvedRuntime(
        ...
        // Install the same analyzer-backed enrichment that CompileCommand / TrailblazeHostYamlRunner
        // use, so scripted-tool (`.ts`) trailmaps resolve here instead of being skipped with a
        // "No ScriptedToolEnrichment was wired" warning — which drops all of that trailmap's
        // waypoints from every `waypoint` command (list/locate/validate/…).
